<div x-data="{
    ...
    @auth
        ...
        <!-- Sidebar Panel -->
        <div class="fixed left-0 top-0 h-screen bg-gray-800 z-40 transition-transform duration-300 ease-in-out flex flex-col"
                    ...
                    <ul class="space-y-4 mt-5">
                        <li>
                            <a href="{{ route('dashboard') }}"
                                class="block py-2 px-4 rounded hover:bg-gray-700 transition-colors duration-200"><span
                        ...
                        <li>
                            <div x-data="{ ticketOpen: true }" class="relative">
                                <button @click="ticketOpen = !ticketOpen"
                                    ...
                                    <svg :class="{'rotate-180': ticketOpen}"
                                ...
                                <div x-show="ticketOpen" x-transition class="flex mt-2 pl-8 space-x-4 items-center">
                                    <a href="{{ route('tickets') }}"
                                        class="relative flex items-center py-2 px-4 rounded hover:bg-gray-700 transition-colors duration-200 text-white ">
                                        Tickets
                                        @if($ticketCount > 0)
                                            <span
                                                class="ml-2 inline-flex items-center justify-center w-6 h-6 text-xs font-bold leading-none text-white bg-red-500 rounded-full">
                                                {{ $ticketCount }}
                                            </span>
                                        @endif
                                    </a>
                            ...
                            <li class="border-t border-gray-700 pt-4">
                                <div x-data="{ adminOpen: true }" class="relative">
                                    <button @click="adminOpen = !adminOpen"
                                        class="w-full flex items-center justify-between px-4 py-2 text-left text-white bg-gray-800 rounded hover:bg-gray-700 transition duration-300">
                                        <span
                                            class="flex items-center font-semibold text-gray-300 tracking-wider uppercase text-sm">
                                        ...
                                        <svg :class="{'rotate-180': adminOpen}"
                                            class="w-4 h-4 transform transition-transform duration-200 text-gray-400"
                                        ...
                                        <div x-data="{ ticketOpen: true }" class="relative">
                                            ...
                                            <div x-show="ticketOpen" x-transition class="mt-2 pl-8 space-y-2">
                                                <a href="{{ route('categories') }}"
                                                    class="block py-2 px-4 rounded hover:bg-gray-700 transition-colors duration-200">Categories
                                                    & Templates</a>
                                            </div>
