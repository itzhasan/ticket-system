<div class="container mx-auto px-4 py-6">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex space-x-2">
                            ...
                            <button wire:click="openMessageModal({{ $ticket->id }})"
            ...
            <div class="mt-3">
                ...
                <form wire:submit.prevent="createTicket">
                    ...
                    <div class="mb-4">
                        ...
                        @error('categoryId') <span class="text-red-500 text-sm">{{ $messag }}</span> @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <select wire:model.live="departmentIds" class="w-full px-3 py-2 border border-gray-300 rounded-md">
                        ...
                        @error('departmentIds') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
        ...
        <div class="relative top-10 mx-auto p-5 border w-11/12 md:w-4/5 lg:w-3/4 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium text-gray-900">Ticket #{{ $selectedTicket->id }} - {{ $selectedTicket->title }}</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <p><strong>Created at:</strong> {{ $selectedTicket->created_at->format('M d, Y H:i') }}</p>
                    </div>
                    <div>
                        <p><strong>Department:</strong></p>
                        @foreach($selectedTicket->departments as $dept)
                        <span class="inline-flex px-2 py-1 text-xs bg-gray-100 text-gray-800 rounded-full mr-1 mb-1">
                            {{ $dept->name }}
                        </span>
                        @endforeach
                    </div>
                ...
                <div class="mb-6">
                    <h4 class="text-md font-medium text-gray-900 mb-3">Ticket</h4>
                    <div class="bg-gray-50 p-4 rounded-md">
                        @foreach($selectedTicket->ticketFieldsValues as $fieldValue)
                        <div class="mb-2">
                            <strong>{{ $fieldValue->templateField->name }}:</strong>
                            <span class="ml-2">{{ $fieldValue->value }}</span>
                        </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="bg-gray-50 p-4 rounded-md max-h-64 overflow-y-auto">
                        ...
                        <div class="mb-3 p-3 bg-white rounded-md shadow-sm">
                            <div class="flex justify-between items-start mb-2">
                                <strong class="text-sm text-gray-900">{{ $message->user->name }}</strong>
    ...
    <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-2/3 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium text-gray-900">Messages - Ticket #{{ $selectedTicket->id }}</h3>
                    <button wire:click="closeMessageModal" class="text-gray-400 hover:text-gray-600">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </button>
                ...
                <!-- Messages Display -->
                <div class="bg-gray-50 p-4 rounded-md max-h-64 overflow-y-auto mb-4">
                    @forelse($messages as $message)
                    <div class="mb-3 p-3 bg-white rounded-md shadow-sm">
                        <div class="flex justify-between items-start mb-2">
                            <strong class="text-sm text-gray-900">{{ $message->user->name }}</strong>
                ...
                <form wire:submit.prevent="sendMessage">
                    <div class="mb-4">
                        ...
                        <textarea wire:model.defer="messageContent" rows="4"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
...
@push('scripts')
<script>
    // Auto-refresh messages every 30 seconds when message modal is open
